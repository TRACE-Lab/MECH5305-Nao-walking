function [u,s]=feedback_linearization_CLF(t,x,D,c_overall,B_overall,t_end_of_previous_step,foot_index,Alpha,current_stance_foot_position)
    ...
    elseif want_bezier==1
        ...
        pphipq = [0;0;dph;zeros(12,1)]*[2/ds_dtheta,zeros(1,29)];
    ...
    e = y+alpha*y_dot;
    %ita = [y;dy];
    %F = [zeros(5,5),eye(5,5);zeros(5,5),zeros(5,5)];
    %G=[zeros(5,5);eye(5,5)];
    P = 0.1*eye(23);
    V = e'*P*e;
    ...
    ub = [];
    x0 = [];
    convergence_rate = 3000;
    options = optimoptions('quadprog','Display','off');
    A = e'*(P'+P);
    b = -convergence_rate*V-e'*(P'+P)*y_dot;
